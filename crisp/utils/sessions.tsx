...
export async function encrypt(payload: any) {
  return await new SignJWT(payload)
    ...
    .setIssuedAt()
    .setExpirationTime(Date.now() + 10 * 60 * 1000) // JWT expiration (1 minute)
    .sign(key);
}
...
// The following function is used because you cant send json data from server to client
export async function isLogged() {
    const auth = await checkAuth();
    return (auth.status == 200) ? "logged" : "not logged"
}